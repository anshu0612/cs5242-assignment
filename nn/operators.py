...
class conv(operator):
    ...
    def forward(self, input, weights, bias):
        ...
        input_img2col = img2col(input_pad, recep_fields_h, recep_fields_w, kernel_h, kernel_w)
        bias1 = np.copy(bias).reshape(bias.shape[0], 1)
        input_conv = np.stack(map(lambda x: np.matmul(weights.reshape(out_channel, -1), x) + bias1,
                                  input_img2col), axis=0)

        output = input_conv.reshape(batch, out_channel, out_height, out_width)
        #####################################################################################
        return output
...
class pool(operator):
    ...
    def forward(self, input):
        ...
        indices = [(a, b) for a in recep_fields_h for b in recep_fields_w]
        receptive_fields_to_col = None
        for i in range(in_channel):
            stacked_channels = np.stack(
                map(lambda x: input_pad[:, i, x[0]:x[0] + pool_height, x[1]:x[1] + pool_width].reshape(batch, -1),
                    indices), axis=-1)
            if receptive_fields_to_col is None:
                receptive_fields_to_col = stacked_channels
            else:
                receptive_fields_to_col = np.concatenate((receptive_fields_to_col, stacked_channels), axis=-1)

        if pool_type == 'max':
            input_pool = np.nanmax(receptive_fields_to_col, axis=1)

        elif pool_type == 'avg':
            input_pool = np.nanmean(receptive_fields_to_col, axis=1)
...
class gru(operator):
    ...
    def forward(self, input, kernel, recurrent_kernel):
        ...
        # code here
        # reset gate
        x_r = sigmoid(np.dot(np.nan_to_num(x), kernel_r) + np.dot(np.nan_to_num(prev_h), recurrent_kernel_r))
        # update gate
        x_z = sigmoid(np.dot(np.nan_to_num(x), kernel_z) + np.dot(np.nan_to_num(prev_h), recurrent_kernel_z))
        # new gate
        x_h = np.tanh(np.dot(np.nan_to_num(x), kernel_h) +
                      np.dot(np.multiply(x_r, np.nan_to_num(prev_h)), recurrent_kernel_h))
        #####################################################################################
    ...
    def backward(self, out_grad, input, kernel, recurrent_kernel):
        ...
        # Reusable grads
        grad0 = out_grad * x_z * (1 - x_z)
        grad1 = (prev_h - x_h) * grad0
        grad2 = out_grad * (1 - x_z) * (1 - np.square(x_h))
        grad3_1 = np.matmul(out_grad * (1 - x_z) * (1 - np.square(x_h)), recurrent_kernel_h.T)
        grad3 = grad3_1 * prev_h * x_r * (1 - x_r)

        # x_grad
        x_a_1 = np.matmul((1 - x_z) * (1 - np.square(x_h)) * out_grad, kernel_h.T) + \
              np.matmul(grad3, kernel_r.T)
        x_a_2 = np.matmul(grad0 * x_h, kernel_z.T)
        x_a = x_a_1 - x_a_2
        x_b = np.matmul(prev_h * grad0, kernel_z.T)
        x_grad = x_a + x_b

        # prev_h_grad
        prev_h_a_1 = x_r * grad3_1 \
                     + np.matmul(grad3_1 * prev_h * x_r * (1 - x_r), recurrent_kernel_r.T)
        prev_h_a_2 = np.matmul(grad0 * x_h, recurrent_kernel_z.T)  # 3, 4
        prev_h_a = prev_h_a_1 - prev_h_a_2
        prev_h_b = out_grad * x_z + np.matmul(prev_h * grad0, recurrent_kernel_z.T)
        prev_h_grad = prev_h_a + prev_h_b

        kernel_z_grad = np.matmul(np.nan_to_num(x.T), grad1)
        kernel_h_grad = np.matmul(np.nan_to_num(x.T), grad2)
        kernel_r_grad = np.matmul(x.T, grad3)

        recurrent_kernel_z_grad = np.matmul(prev_h.T, grad1)
        recurrent_kernel_h_grad = np.matmul((prev_h * x_r).T, grad2)
        recurrent_kernel_r_grad = np.matmul(prev_h.T, grad3)
        #####################################################################################
        in_grad = [x_grad, prev_h_grad]
        kernel_grad = np.concatenate([kernel_z_grad, kernel_r_grad, kernel_h_grad], axis=-1)
        r_kernel_grad = np.concatenate([recurrent_kernel_z_grad, recurrent_kernel_r_grad, recurrent_kernel_h_grad],
                                       axis=-1)
