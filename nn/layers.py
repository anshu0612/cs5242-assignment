...
class BiRNN(Layer):
    ...
    def forward(self, input):
        ...
        x_rev = self._reverse_temporal_data(input, mask)
        ht_rev = self.backward_rnn.forward(x_rev)
        ht_rev_roll = self._reverse_temporal_data(ht_rev, mask)

        output = np.concatenate((ht, ht_rev_roll), axis=2)
        #####################################################################################
